...
router.post('/send_code1', function(req, res, next) {
  var phone, region;
...
router.post('/verify_code1', function(req, res, next) {
...
router.post('/check_phone_available1', function(req, res, next) {
  var phone, region;
  region = req.body.region;
  phone = req.body.phone;
  if (!validator.isMobilePhone(phone.toString(), regionMap[region])) {
...
router.post('/login111', function(req, res, next) {
  console.log('dnetlogin');
  ...
  return User.findOne({
    ...
    attributes: ['passwordHash', 'passwordSalt', 'rongCloudToken']
  }).then(function(user) {
...
router.post('/get_nickname1', function(req, res, next) {
  var userId, id;
  userId = req.body.userId;
  id = Utility.decodeIds(userId)
  return User.findOne({
    where: {
      id: id
    },
...
router.post('/get_user_groups1', function(req, res, next) {
  var userId, id;
  userId = req.body.userId;
  id = Utility.decodeIds(userId)
  return Cache.get("user_groups_" + id).then(function(groups) {
    if (groups) {
      ...
      return GroupMember.findAll({
        ...
        var results;
        results = Utility.encodeResults(groups, [['group', 'id']]);
...
router.post('/logout1', function(req, res) {
  res.clearCookie(Config.AUTH_COOKIE_NAME);
  return res.send(new APIResult(200));
});

router.post('/reset_password1', function(req, res, next) {
...
router.post('/change_password1', function(req, res, next) {
...
router.post('/set_nickname1', function(req, res, next) {
...
router.post('/set_portrait_uri1', function(req, res, next) {
...
router.post('/add_to_blacklist1', function(req, res, next) {
...
router.post('/remove_from_blacklist1', function(req, res, next) {
  var currentUserId, encodedFriendId, friendId, timestamp;
  friendId = req.body.friendId;
  encodedFriendId = req.body.encodedFriendId;
  currentUserId = Session.getCurrentUserId(req);
...
router.post('/upload_contacts1', function(req, res, next) {
  var contacts;
  contacts = req.body;
  return res.status(404).send('Not implements.');
});

router.get('/get_token1', function(req, res, next) {
  return User.findById(Session.getCurrentUserId(req, {
...
router.get('/get_image_token1', function(req, res, next) {
  var putPolicy, token;
...
router.get('/get_sms_img_code1', function(req, res, next) {
...
router.get('/blacklist1', function(req, res, next) {
...
router.get('/sync/:version1', function(req, res, next) {
...
router.get('/batch1', function(req, res, next) {
  var ids;
  ids = req.query.id;
...
router.get('/find/:region/:phone1', function(req, res, next) {
  var phone, region;
  region = req.params.region;
  phone = req.params.phone;
  if (!validator.isMobilePhone(phone, regionMap[region])) {

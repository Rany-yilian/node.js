...
GROUP_OPERATION_STOPADDUSER = 'Stopadduser';
...
router.post('/create', function(req, res, next) {
  ...
  //start 2s
  return Group.update({
    groupId: Utility.encodeId(group.id)
  }, {
    where: {
      id: group.id
    }
...
router.post('/add', function(req, res, next) {
  ...
  return Group.getInfo(groupId).then(function(group) {
	...
	if (group.dataValues.limitNum) {
      return res.status(400).send(new APIResult(4001, Utility.encodeResults({
            msg: "已被群主限制！"
          })));
    }
...
router.post('/limit_flag', function(req, res, next) {
  var groupId, userId, userId1;
  groupId = req.body.groupId;
  ...
  return Group.getInfo(groupId).then(function(group) {
	...
	userId1 = Utility.decodeIds(userId);
	if(group.creatorId != userId){
	  return res.status(404).send('creatorId error.');
	}
	var limitFalg = "0";
	if(group.dataValues.limitNum == 0){// group.dataValues.limitNum
		limitFalg = "2";
	}else{
		limitFalg = "1";
	}
...
router.post('/limit_user', function(req, res, next) {
  var groupId, userId, limitFalg, limitFalg1;
  groupId = req.body.groupId;
  userId  = req.body.userId;
  limitFalg = req.body.limitFalg;
  return Group.getInfo(groupId).then(function(group) {
    var memberCount;
    if (!group) {
      return res.status(404).send('Unknown group.');
    }
	if(limitFalg != 1 && limitFalg != 2){
	  return res.status(404).send('limitFlag error.');
	}
	switch(limitFalg){
		case '1':
			limitFalg1 = 1; break;
...
router.post('/show_group_bulletin111', function(req, res, next) {
  var groupId;
  groupId = req.body.groupId;
  return Group.getInfo(groupId).then(function(group) {
    var memberCount;
...
router.post('/group_report', function(req, res, next) {
  var groupId, userId;
  groupId = req.body.groupId;
  userId  = req.body.userId;
  return Group.getInfo(groupId).then(function(group) {
...
router.post('/join', function(req, res, next) {
  ...
  return Group.getInfo(groupId).then(function(group) {
    ...
    if (group.limitNum==1) {
          return res.status(400).send("group limit join");
    }
    return sequelize.transaction(function(t) {
      return Promise.all([
        Group.update({
...
router.post('/dismiss_admin', function(req, res, next) {
  var currentUserId, encodedGroupId, groupId, timestamp;
  groupId = req.body.groupId;
  encodedGroupId = req.body.encodedGroupId;
